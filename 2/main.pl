/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   Wumpus World - by Alexander Cs_ka Roque
   Written in S_o Paulo, Brazil, April 2018
   This was developed as a exercise for the Artificial Intelligence
   Course at Universidade de S_o Paulo.
   ...
   and more importantly, Pilar Pozos, Edgardo Yescas and Jacob V_squez
...
hasGold(do(grab,S)) :- w_Gold(R), hunter(R,_,S). %If hunter use grab at the same room as gold, pick it up
hasGold(do(_,S)) :- hasGold(S). %If hunter had the gold before, still has now
...
wall(R,do(A,S)) :- perceiveBump(do(A,S)), hunter(Rh,Dh,do(A,S)), getForwardRoom(Rh,Dh,R).
wall(R,do(_,S)) :- wall(R,S). %Agent remembers bumping into walls
visited(r(1,1),s0).
visited(R,do(A,S)) :- hunter(R,_,do(A,S)).
visited(R,do(_,S)) :-  visited(R,S). %Agent remembers all rooms he has visited
gold(R,do(A,S)) :- perceiveGlitter(do(A,S)),hunter(R,_,do(A,S)).
gold(R,do(_,S)) :- gold(R,S). %Once gold is found, remember where it was
breeze(R,s0) :- perceiveBreeze(s0),hunter(R,_,s0).
breeze(R,do(A,S)) :- perceiveBreeze(do(A,S)),hunter(R,_,do(A,S)).
breeze(R,do(_,S)) :- breeze(R,S). %Remember where breeze was perceived
stench(R,s0) :- perceiveStench(s0),hunter(R,_,s0).
stench(R,do(A,S)) :- perceiveStench(do(A,S)),hunter(R,_,do(A,S)).
stench(R,do(_,S)) :- stench(R,S). %Remember where stench was perceived
heardWumpusScream(s0) :- false.
heardWumpusScream(do(A,S)) :- perceiveScream(do(A,S)).
heardWumpusScream(do(_,S)) :- heardWumpusScream(S). %Represent actual knowledge of hearing the wumpus scream (and hopefully die)
...
possiblePit(R,S) :- \+visited(R,S), getAdjacentRooms(R,LA), trimNotVisited(LA,S,LT), checkBreezeList(LT,S).
checkBreezeList([],_).
...
possibleWumpus(R,S) :- certainWumpus(R,S). %a certain Wumpus is also a possible Wumpus    
possibleWumpus(R,S) :- \+visited(R,S), getAdjacentRooms(R,LA), trimNotVisited(LA,S,LT), checkStenchList(LT,S).
checkStenchList([],_).
...
% be there.
certainWumpus(RW,do(_,S)) :- certainWumpus(RW,S). %Check first Wumpus certainty before, because if he is killed and the hunter visits the space the rest of the code returns false
...
heuristic(S,H) :-
    ...
    heuristicPickAction(S,P,R,H).

heuristicPickAction(S,_,_,exitCave) :- hasGold(S). %If hunter has gold, proceed to exit
heuristicPickAction(S,_,_,grabGold) :- \+hasGold(S), gold(_R,S). %If doesn't have gold but knows where it is, go get it
heuristicPickAction(S,_,_,shootWumpus) :- certainWumpus(_RW,S),hasArrow(S),\+heardWumpusScream(S). %If is certain of where the Wumpus is, has arrow and Wumpus is alive, shoot him
heuristicPickAction(_,P,R,move(R)) :- P < 100. %Only move if best room to explore is not dangerous
heuristicPickAction(_,_,_,exitCave).  %If no rooms to explore, exit cave
...
doActions(grabGold,do(grab,SI),S0) :- gold(R,S0), doMove(R,SI,S0). %Move and then grab
doActions(shootWumpus,do(shoot, SI),S0) :- certainWumpus(RW,S0), doFace(RW,SI,S0). %Face Wumpus and shoot
...
printGoldState(S, B) :- gold(B, S), !, format('g').
printGoldState(S, B) :- visited(B, S), !, format('v').
printGoldState(_S, _B) :- !, format('?').

printPitState(S, B) :- certainPit(B, S), !, format('P').
printPitState(S, B) :- possiblePit(B, S), !, format('p').
printPitState(S, B) :- breeze(B, S), !, format('b').
printPitState(_S, _B) :- !, format(' ').

printWumpusState(S, B) :- certainWumpus(B, S), wumpusAlive(S), !, format('U').
printWumpusState(S, B) :- certainWumpus(B, S), \+wumpusAlive(S), !, format('F').
printWumpusState(S, B) :- possibleWumpus(B, S), !, format('u').
printWumpusState(S, B) :- stench(B, S), !, format('|').
printWumpusState(_S, _B) :- !, format(' ').
